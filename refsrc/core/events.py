...
from core.runtime import runtime_info, GlobalState, get_global_state
...
def startup(app: FastAPI) -> Callable:
    ...
    async def app_start() -> None:
        ...
        state.v = "3.10"
        state.cache = await sys_cache()
        state.code = await code_cache()
...
async def app_lifespan(app: FastAPI):
    """
    同一个应用中，lifespan和startup、shutdown方法不能同时使用，为了兼容性考虑，需要使用事件驱动器
    :param app:
    :return:
    """
